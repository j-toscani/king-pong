...
async function initSocket(httpServer) {
  ...
  io.on("connection", socket => {
    ...
    function drawLoop(game) {
      ...
      const newGameState = calculateNewGameState(game, timeSinceLastDraw);
      if (game.global.play) {
        ...
        io.to(room).emit("game ended", "game ended");
    ...
    socket.on("first frame", () => {
      ...
      const player1 = idsOfUsersInRoom[0];
      const player2 = idsOfUsersInRoom[1];
